...
export class RealTimeEventsService implements IEventService {
  ...
  private formatLocation(location: string | Coordinates): { location: string; lat?: string; lon?: string } {
      ...
      const response = await APIErrorHandler.withRetry(
        async () => {
          ...
          const queryParams = {
            q: params.query,
            ...locationParams,
            radius: (params.location as Coordinates).radius?.toString() || '50', // Default to 50km radius
  ...
  private transformEvent(event: any): Event | null {
    try {
